...
class EntryWindow(QMainWindow, MainWindow):
    ...
    def change_layout(self, index):
        ...
        for i in range(self.layout[0] * self.layout[1]):
            ...
            self.new_buttonlist[i].resize(int(width), int(height))
            self.new_buttonlist[i].move(int(80 + i % self.layout[1] * (630.0 / self.layout[1])),
                                        int(120 + i // self.layout[1] * (270.0 / self.layout[0])))
            font = QFont()
            font.setFamily("Times New Roman")
            font.setPointSize(20)
