from mola.layouts import DefaultStructureImage, HighlightStructureImage, PropertyStructureImage, Figure
...
def designed(file_parent_path, temp_parent_path, save_parent_path):
    ...
    t0950.set_state(rotate=[0, 60, 255], inner_align=True, target="T0950")
    t0950.set_shape(representation_plan=[("model:T0950", "cartoon")])
    ...
    predict.set_state(rotate=[0, 60, 255], inner_align=True, target="T0950")
    predict.set_shape(representation_plan=[("model:Predict", "cartoon")])
    ...
    case = Figure(manuscript_format="Science", occupied_columns=2, aspect_ratio=(1, 2))
    ...
    structure = PropertyStructureImage(structure_paths=[file_parent_path + "T0950.pdb",
                                                        file_parent_path + "Predict.pdb"])
    structure.set_state(rotate=[0, 60, 255], inner_align=True, target="T0950")
    structure.set_shape(representation_plan=[("all", "cartoon")])
